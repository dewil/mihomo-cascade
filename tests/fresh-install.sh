...
# ПРАВКА 22.09.2026, сделана основным агентом, а не автором реализации:
#   прежняя регулярка требовала запятую или начало строки перед именем каталога
#   ((^|,) после .*), а в середине строки ^ не совпадает никогда - то есть простое
#   StateDirectory=mihomo она не принимала в принципе, хотя контракт в шапке этого
#   теста его прямо разрешает. Реализация подгонялась под нее, дописывая путь
#   состояния комментарием в установленный юнит; убрано и то, и другое.
