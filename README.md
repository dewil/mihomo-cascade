...
- `mihomo-refresh.timer` раз в 2 минуты запускает `mihomo-refresh` — пересборка конфига + hot reload через REST API.
  ...
  - Наложение прогонов исключает сам systemd: второй экземпляр `mihomo-refresh.service` не стартует, пока активен первый (прежнему крону для этого нужен был `flock`). Страховка от зависшего прогона - `TimeoutStartSec=300`.
