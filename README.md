...
Сработал порог — сборка отменяется (код `3`), файл провайдера и `config.yaml` остаются прежними, `mihomo-refresh` завершается штатно и пишет предупреждение в journal. Если ноду вывели специально и список должен сократиться:
