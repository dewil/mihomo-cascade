...
## Клиентское применение на Linux: прокси для локальной сети

Linux-машина с mihomo может быть прокси для телефона и других устройств в LAN.
В `/etc/mihomo/config.base.yaml` задайте адрес этой машины в локальной сети
(пример — `192.168.1.10`):
...
Порт SOCKS5 `7891` уже предусмотрен базовым конфигом. Примените изменения через
`mihomo-refresh`. В Telegram на телефоне в настройках прокси выберите **SOCKS5**,
укажите адрес сервера `192.168.1.10` и порт `7891`; телефон должен иметь доступ к LAN.
Установщик обновляет `config.base.yaml` из репозитория, поэтому при обновлении
локальные настройки нужно сохранить и применить заново.

**`bind-address: "*"` на машине с публичным адресом недопустим без ограничения
фаерволом:** это открытый SOCKS5 на всех интерфейсах, доступный посторонним,
злоупотребления и нарушение правил хостера. Используйте адрес LAN, как выше,
либо разрешите фаерволом доступ к прокси только из доверенной локальной сети
(учитывая также mixed-порт `7890` и IPv6). За NAT звездочка безобидна, если нет
проброса портов и прямого доступа извне, в том числе по IPv6: прокси остается
доступен только локальной сети.
