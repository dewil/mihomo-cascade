...
- `mihomo-refresh.timer` раз в 2 минуты запускает `mihomo-refresh` — пересборка конфига + hot reload через REST API.
  ...
  - Наложение прогонов исключено двумя механизмами сразу. systemd не стартует второй экземпляр `mihomo-refresh.service`, пока активен первый, - но только своего юнита. Поэтому `ExecStart` дополнительно берет `flock -n -E 0 /run/mihomo-refresh.lock` (тот же лок, что брал прежний крон): под него попадают и старый cron-прогон в окне миграции, и ручной запуск `mihomo-refresh` - иначе они делили бы с таймером `config.yaml.autobak`, `*.new` и `routing-rules.yaml.tmp`. `-E 0` означает, что занятый лок - штатный пропуск, а не `failed` в journal. Страховка от зависшего прогона - `TimeoutStartSec=300`.
